...
TEST_CASE("Pick item that is qualified"){
    ...
    bool succesful = warehouse.pickItems("Books", 50);
    //Should succeed
    REQUIRE(succesful);

    //Check if item count changed succesfully
    REQUIRE(warehouse.shelves[0].pallets[0].getItemCount() == 0);
...
TEST_CASE("Pick item that is not qualified"){
    // Construct warehouse with shelf of books.
    Warehouse warehouse = Warehouse();
    Shelf shelf1 = Shelf();
    shelf1.pallets = {
    ...
    bool succesful = warehouse.pickItems("Books", 60);
    //Should succeed
    REQUIRE(!succesful);

    //Check if item count did not change succesfully
    REQUIRE(warehouse.shelves[0].pallets[0].getItemCount() == 20);
    REQUIRE(warehouse.shelves[0].pallets[1].getItemCount() == 20);
